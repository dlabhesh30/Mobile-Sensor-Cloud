...
%>			
          ...
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title">Hub Details</h3>
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAw2jBXNkaAns3t3N2K3c1EjNBb-H5nYBQ&callback=initMap"></script>
   ...
   String sql = "select hubname from hubtable";
   ResultSet rs = stmt.executeQuery(sql);
   while (rs.next()){%>
   <script>
     locations_name.push("<%=rs.getString("hubname")%>");
     locations_address.push("<%= rs.getString("hubname")%>");
     </script>  
   <%   
   }  
%>  
...
for (i=0;i<locations_address.length;i++){
	locations_address[i] = locations_address[i].replace("Hub_", "");
}
for(i=0;i<locations_address.length;i++) {
