...
Connection con = null;
	...
	try{  
		...
		String query = "SELECT sensorid,totalRequests AS requests FROM sensortable WHERE username='"+username+"' AND hubname='"+location+"' AND State='Running'"+
						"AND totalRequests=(SELECT max(totalRequests) from sensortable WHERE username='"+username+"' AND hubname='"+location+"')";
		ResultSet rs = st.executeQuery(query);
		int totalRequests=0;
		while (rs.next()) {
			sensorname = rs.getString("sensorid");
			totalRequests = rs.getInt("requests");
		}
		query= "UPDATE sensortable SET totalRequests = totalRequests-1 WHERE sensorid='"+sensorname+"'";
   		st.executeUpdate(query);
		%>
	<div class="box box-info">
		<div class="box-header with-border">
              <h3 class="box-title">Patrol request on <%=sensorname %> is stopped</h3>
...
<div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title">Patrol requests allocation to sensors in <%=location%></h3>
...
<%		
			query = "select * from sensortable where username='"+username+"' and hubname='"+location+"'";
